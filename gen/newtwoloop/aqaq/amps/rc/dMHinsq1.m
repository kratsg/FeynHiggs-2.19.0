{dMHinsq1 -> -(MHin2*((1 - inputMHp)*dZHiggs1gl[3, 3] + 
 ...
 {{dMHinsq1[-1] -> 
    -((Nc*((1 - inputMHp)*(MB2*DiagMark[MB]*((g[hBBL, 3]*g[hBBR, 3])/4 + 
            (g[hBBL, 3] + g[hBBR, 3])^2/8) + MT2*DiagMark[MT]*
           ((g[hTTL, 3]*g[hTTR, 3])/4 + (g[hTTL, 3] + g[hTTR, 3])^2/8)) + 
        (inputMHp*DiagMark[MT, MB]*(g[HpTBL, 1]*(MT2*gC[HpTBL, 1] + 
             MB*MT*gC[HpTBR, 1]) + g[HpTBR, 1]*(MB*MT*gC[HpTBL, 1] + 
             MT2*gC[HpTBR, 1]) + MB2*(g[HpTBL, 1]*gC[HpTBL, 1] + 
             g[HpTBR, 1]*gC[HpTBR, 1])))/8 - 
   ...
   dMHinsq1[0] -> 
    -((Nc*((1 - inputMHp)*(MB2*DiagMark[MB]*((B0fin[0, 0, MB2]*g[hBBL, 3]*
              g[hBBR, 3])/4 - (g[hBBL, 3] + g[hBBR, 3])^2/8) + 
          MT2*DiagMark[MT]*((B0fin[0, 0, MT2]*g[hTTL, 3]*g[hTTR, 3])/4 - 
            (g[hTTL, 3] + g[hTTR, 3])^2/8)) + 
        (MB2*(inputMHp*B0fin[0, 0, MB2]*DiagMark[MT, MB]*
             (g[HpTBL, 1]*gC[HpTBL, 1] + g[HpTBR, 1]*gC[HpTBR, 1]) + 
            (1 - inputMHp)*DiagMark[MB]*(g[hBBL, 3] + g[hBBR, 3])^2*
             Re[B0fin[0, 0, MB2]]) + (1 - inputMHp)*MT2*DiagMark[MT]*
           (g[hTTL, 3] + g[hTTR, 3])^2*Re[B0fin[0, 0, MT2]] + 
          inputMHp*DiagMark[MT, MB]*(g[HpTBL, 1]*(MT2*gC[HpTBL, 1] + 
              MB*MT*gC[HpTBR, 1]) + g[HpTBR, 1]*(MB*MT*gC[HpTBL, 1] + 
              MT2*gC[HpTBR, 1]))*Re[B0fin[0, MB2, MT2]])/8 - 
   ...
   dMHinsq1[1] -> 
    -((Nc*(MB2*((inputMHp*B0del[0, 0, MB2]*DiagMark[MT, MB]*
          ...
          ((1 - inputMHp)*DiagMark[MB]*(g[hBBL, 3] + g[hBBR, 3])^2*
            (B0fin[0, 0, MB2] - Re[B0del[0, 0, MB2]]))/8) + 
        (1 - inputMHp)*((MB2*B0del[0, 0, MB2]*DiagMark[MB]*g[hBBL, 3]*
             g[hBBR, 3] + MT2*B0del[0, 0, MT2]*DiagMark[MT]*g[hTTL, 3]*
             g[hTTR, 3])/4 - (MT2*DiagMark[MT]*(g[hTTL, 3] + g[hTTR, 3])^2*
            (B0fin[0, 0, MT2] - Re[B0del[0, 0, MT2]]))/8) + 
        (inputMHp*DiagMark[MT, MB]*(g[HpTBL, 1]*(MT2*gC[HpTBL, 1] + 
             MB*MT*gC[HpTBR, 1]) + g[HpTBR, 1]*(MB*MT*gC[HpTBL, 1] + 
             MT2*gC[HpTBR, 1]))*Re[B0del[0, MB2, MT2]])/8 - 
