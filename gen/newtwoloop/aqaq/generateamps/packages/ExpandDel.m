...
DelSeries[x_] := dp[0][Collect[x, {del, _RC}, dc]] /; FreeQ[x, p_Plus^(_?Negative) /; !FreeQ[p, del]]
...
Map[(DC[0, #] = #) &, {SEHmHp2, SEA0A02, -Tadh02, -TadHH2, -TadA02}]

Format[DC[n_, x_]] := "DC"[n, SequenceForm["-", LeafCount[x], "-"]]


Conjugate[RC[i___, x_]] ^:= RC[i, Conjugate[x]]
Unprotect[Plus]; Re[RC[i___, x_] + y_] ^:= RC[i, Re[x]] + Re[y]; Im[RC[i___, x_] + y_] ^:= RC[i, Im[x]] + Re[y]; Protect[Plus];
