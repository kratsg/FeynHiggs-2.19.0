...
RenConst[dg[hTTL, hid_]] := 
 Sum[g[hTTL, sumhid] dZHiggs1gl[sumhid, hid]/2, {sumhid, 4}] + 
  dht1/ht g[hTTL, hid]

RenConst[dg[hTTR, hid_]] := 
 Sum[g[hTTR, sumhid] dZHiggs1gl[sumhid, hid]/2, {sumhid, 4}] + 
  dht1/ht g[hTTR, hid]

RenConst[dg[hBBL, hid_]] := 
 Sum[g[hBBL, sumhid] dZHiggs1gl[sumhid, hid]/2, {sumhid, 4}] + 
  dhb1/hb g[hBBL, hid]

RenConst[dg[hBBR, hid_]] := 
 Sum[g[hBBR, sumhid] dZHiggs1gl[sumhid, hid]/2, {sumhid, 4}] + 
  dhb1/hb g[hBBR, hid]

RenConst[dg[HpTBL, hid_]] := 
 Sum[g[HpTBL, sumhid] dZbarHiggs1gl[sumhid + 4, hid + 4]/2, {sumhid, 
    2}] + dht1/ht g[HpTBL, hid]

RenConst[dg[HpTBR, hid_]] := 
 Sum[g[HpTBR, sumhid] dZbarHiggs1gl[sumhid + 4, hid + 4]/2, {sumhid, 
    2}] + dhb1/hb g[HpTBR, hid]

RenConst[dg[hStSt, hid_, s1_, s2_]] := 
 Sum[g[hStSt, sumhid, s1, s2] dZHiggs1gl[sumhid, hid]/2, {sumhid, 
    4}] + dht1/
    Sqrt[2] (USf[s2, 2, 3, 3] USfC[s1, 1, 3, 
       3] (ZHu[hid] Af[3, 3, 3] - Conjugate[ZHd[hid]] MUEC) + 
     USf[s2, 1, 3, 3] USfC[s1, 2, 3, 
       3] (Conjugate[ZHu[hid]] AfC[3, 3, 3] - 
        ZHd[hid] MUE) + (ZHu[hid] + 
        Conjugate[ZHu[hid]]) (USf[s2, 1, 3, 3] USfC[s1, 1, 3, 3] + 
        USf[s2, 2, 3, 3] USfC[s1, 2, 3, 3]) Mf[3, 3]) + 
  ht/Sqrt[2] (USf[s2, 2, 3, 3] USfC[s1, 1, 3, 
       3] (ZHu[hid] dAf1[3, 3, 3] - 
        Conjugate[ZHd[hid]] Conjugate[dMUE1]) + 
     USf[s2, 1, 3, 3] USfC[s1, 2, 3, 
       3] (Conjugate[ZHu[hid]] Conjugate[dAf1[3, 3, 3]] - 
        ZHd[hid] dMUE1) + (ZHu[hid] + 
        Conjugate[ZHu[hid]]) (USf[s2, 1, 3, 3] USfC[s1, 1, 3, 3] + 
        USf[s2, 2, 3, 3] USfC[s1, 2, 3, 3]) dMf1[3, 3])
...
RenConst[dv1] := v (1/2 dMWsq1MW2 + dSW1/SW)
...
RenConst[dMHinsq1] := inputMHp (MassRC[S[5] @ 0] - MHin2 dZHiggs1gl[5,5]) +
  (1 - inputMHp) (MassRC[S[3] @ 0] - MHin2 dZHiggs1gl[3,3]);
...
nomix = {
g[hTTL, hid_] :> KroneckerDelta[hid,2] ht/Sqrt[2],
g[hTTR, hid_] :> KroneckerDelta[hid,2] ht/Sqrt[2],
g[hBBL, hid_] :> KroneckerDelta[hid,1] hb/Sqrt[2],
g[hBBR, hid_] :> KroneckerDelta[hid,1] hb/Sqrt[2]
};
  
RenConst[dZ22H1gl] := dZ22H1fingl + UVDivergentPart[FieldRC[S[2] @ 0] /. nomix]

RenConst[dZ11H1gl] := dZ11H1fingl + UVDivergentPart[FieldRC[S[1] @ 0] /. nomix]

RenConst[dAf1[4, j1_, j1_]] :=
Block[ {dMSd1sq, dMSd2sq, dYd, dMUEdr, dTBdr},
  ...
  dTBdr = RenConst[dTB1] //. {
    d:dZ11H1gl|dZ22H1gl :> UVDivergentPart[RenConst[d]],
    d:dZ12H1gl|dZ21H1gl|dTB1fingl -> 0 };
  UVDivergentPart[
...
hb = cTAG[Yuk] MB/v/CB
*)
MTMW = cTAG[Yuk] SB ht Sqrt[2] SW/EL;
MBMW = cTAG[Yuk] CB hb Sqrt[2] SW/EL;
...
glcoup[expr_] := Collect[
  expr /. {
    ...
    MB2 -> MBMW^2 MW2,
    MB -> MBMW MW,
    m:MSf2[1|2,3,3] :> MTMW^2 MW2/MT2 m,
    m:MSf[1|2,3,3] :> MTMW MW/MT m,
    m:MSf2[1|2,4,3] :> MBMW^2 MW2/MB2 m,
    m:MSf[1|2,4,3] :> MBMW MW/MB m},
  {_cTAG}, MagicSimplify[#, Feedback -> False]& ]
...
Map[(RealQ[#] = True) &,
    {_cTAG, MMUE2, M0, M02, MHptree2, MA0tree2, MHp2, MA02, MW2, MZ2, v, inputMHp, inputMA0, del,
       SW, CW, SW2, CW2, MGl, MT, MB, _MSf, MGl2, MT2, MB2, _MSf2, UCSf[1 | 2, 1 | 2, __]}];

RealQ[Times[x_, y__]] := RealQ[x] && RealQ[Times[y]];
RealQ[Power[x_, i_?IntegerQ]] := RealQ[x];

resimp[x_] := x /. (a : Re | Im)[y_] :> a[Expand[y]] /.
      (a : Re | Im)[y_] :> a[Numerator[y]]/Denominator[y] /; RealQ[Expand[Denominator[y]]] /.
      (a : Re | Im)[y_] :> Refine[a[y]];
...
(*
  pre = pre /.
    ...
    Alfa -> 0;
*)
  pre = resimp @ USfSimplify @ Expand[pre, _USf | _USfC];
  pre
]
